...
# 1. Prevent overlapping mesh intersections from welding joints shut
model.opt.disableflags |= mujoco.mjtDisableBit.mjDSBL_CONTACT

# 2. Erase any hidden brakes, damping, or friction hardcoded in the URDF file text
...
# Joint-specific controller gains tailored to link masses 
kp = np.array([800.0, 1000.0, 600.0, 60.0, 60.0, 30.0])
kv = np.array([ 140.0,  180.0, 100.0, 12.0, 12.0,  6.0])

step_index = 0
total_energy_squared_torque = 0.0
sim_time = 0.0

print("Starting Stable, Corrected MuJoCo Simulation...")
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() and step_index < total_simulation_steps:
        ...
        # FIX: Reset forces OUTSIDE the sub-stepping loop so channels aren't dropped chattering
        ...
        for _ in range(substeps):
            ...
            # Element-wise PD torque calculation
            tau_pd = kp * (target_q - current_q) + kv * (target_v - current_v)
            
            # Direct mapping to isolated generalized coordinate targets
            for j in range(6):
                data.qfrc_applied[joint_dof_adrs[j]] = tau_pd[j]
            ...
            tau_squared_accum += np.sum(tau_pd**2)
